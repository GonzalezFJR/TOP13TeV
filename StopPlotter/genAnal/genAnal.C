...
TString path = "/pool/ciencias/HeppyTreesDR80X/v1/";
...
const TString process[nprocess]     = {"ttbar_Powheg", "ttbar_MLM", "stop_200_25", "stop_250_75"};
const TString processfile[nprocess] = {"TTbar_Powheg_ext_0", "TTJets_MLM_0", "T2tt_150to250_0", "T2tt_150to250_0"};
...
void loop(Int_t iprocess = 0){
  ...
  int p5 = int(float(nEntries)/20);
    ...
    mett.SetPtEtaPhiM(met_pt, 0, met_phi, 0);
...
float getMT2ll(){
	...
	TLorentzVector pmet;
	pmet.SetPtEtaPhiM(met_pt, 0., met_phi, 0.);
...
TLorentzVector getPtllb(){
	TLorentzVector pmet; pmet.SetPtEtaPhiM(met_pt, 0., met_phi, 0.);
	TLorentzVector plep1, plep2;
	plep1.SetPtEtaPhiM(genLep_pt[0], genLep_eta[0], genLep_phi[0], genLep_mass[0]);
	plep2.SetPtEtaPhiM(genLep_pt[1], genLep_eta[1], genLep_phi[1], genLep_mass[1]);
